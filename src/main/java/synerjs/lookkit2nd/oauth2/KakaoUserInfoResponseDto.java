...
public class KakaoUserInfoResponseDto {
    ...
    public User toEntity(KakaoUserInfoResponseDto userInfo) {
        // 생년월일 변환
        String dateString = userInfo.getKakaoAccount().getBirthYear()+userInfo.getKakaoAccount().getBirthDay();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd");
        LocalDate birthdata = LocalDate.parse(dateString, formatter);

        User user = User.builder()
                .userUuid("kakao_"+userInfo.getId())
                .userName(userInfo.getKakaoAccount().getName())
                .gender(userInfo.getKakaoAccount().getGender().equals("male")?"M":"F")
                .email("kakao_"+userInfo.getKakaoAccount().getEmail())
