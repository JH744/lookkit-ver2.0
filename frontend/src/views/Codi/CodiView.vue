    ...
    <div class="details-section">
      <div>
        ...
        <div class="price-section">
          <span class="price" v-if="codi.codiPrice !== undefined"
            >{{ codi.codiPrice.toLocaleString() }}원</span
          >
        ...
        <div class="rental-section">
          ...
          <Datepicker
            ...
            @update:model-value="validateRentalStartDate"
            class="rental-input"
            placeholder="대여일"
          />
          <Datepicker
        ...
        <div
          v-if="rentalStartDate && rentalEndDate"
          class="selected-rental-info"
        >
          <div class="selected-rental-summary">
            ...
            <p>
              <strong style="font-size: 1.5em"
                >총 대여 금액 {{ formattedTotalPrice }}</strong
              >
            </p>
          </div>
...
const fetchImages = async () => {
  try {
    ...
    const thumbnailRef = firebaseRef(
      firebaseStorage,
      `lookkit/codi/0${codiId.value}/${codiId.value}_thumbnail.webp`
    );
    ...
    for (let i = 1; i <= 6; i++) {
      try {
        const detailRef = firebaseRef(
          firebaseStorage,
          `lookkit/codi/0${codiId.value}/${codiId.value}_detail_${i}.webp`
        );
        const detailUrl = await getDownloadURL(detailRef);
        detailImageUrls.value.push(detailUrl);
      } catch (error) {
        console.log(
          `디테일 이미지 ${i} 가져오기 실패 (존재하지 않을 수 있음)`,
          error
        );
      }
    }
...
const formattedRentalStartDate = computed(() =>
  formatDate(rentalStartDate.value)
);
...
const addToCart = async () => {
  try {
    ...
    const formattedStartDate = getFormattedDate(
      new Date(rentalStartDate.value)
    );
