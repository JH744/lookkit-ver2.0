    ...
    <div class="details-section">
      <div>
        ...
        <div class="price-section">
          <span class="price" v-if="product.productPrice !== undefined"
            >{{ product.productPrice.toLocaleString() }}원</span
          >
          ...
          <div class="selected-summary">
            ...
            <p>
              <strong style="font-size: 1.5em"
                >총 구매 금액: {{ formattedTotalPrice }}</strong
              >
            </p>
          </div>
        </div>
...
const fetchImages = async () => {
  try {
    ...
    const thumbnailRef = firebaseRef(
      firebaseStorage,
      `lookkit/products/0${productId.value}/${productId.value}_thumbnail.webp`
    );
    ...
    for (let i = 1; i <= 6; i++) {
      try {
        const detailRef = firebaseRef(
          firebaseStorage,
          `lookkit/products/0${productId.value}/${productId.value}_detail_${i}.webp`
        );
...
const formattedTotalPrice = computed(
  () => `${totalPrice.value ? totalPrice.value.toLocaleString() : "0"}원`
);
