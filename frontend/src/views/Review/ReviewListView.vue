<template>
      ...
      <tbody>
        <tr v-for="review in reviews" :key="review.reviewId">
          <td>
            ...
            <img :src="review.thumbnailUrl || '/images/placeholder.png'" alt="상품 이미지" class="product-thumbnail" @click="navigateToProductDetail(review.productId)"/>
          ...
          <td><button @click="showDeleteConfirmModal(review.reviewId)" class="delete-btn">삭제</button></td>
        </tr>
      </tbody>
    </table>
  </template>
  ...
  const loadReviews = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/api/reviews/list/${authStore.user.userId}`);
      reviews.value = response.data;
  
      for (let review of reviews.value) {
        if (review.productId) {
          fetchImageForReview(review);
        }
      }
  ...
  const fetchImageForReview = async (review) => {
    const storagePath = `lookkit/products/0${review.productId}/${review.productId}_thumbnail.webp`;
    try {
