    ...
    <div class="second-grid">
      ...
      <section v-if="selectedProduct" class="review-section">
        ...
        <div class="product-details">
          <img :src="selectedProduct.thumbnailUrl || '/images/placeholder.png'" alt="상품 이미지" class="product-image" />
          <div>
            <p class="product-name">{{ selectedProduct.brandName }}</p>
            <p class="product-model">{{ selectedProduct.productName }}</p>
            <p class="product-price">{{ selectedProduct.productPrice.toLocaleString() }}원 / 수량 {{ selectedProduct.quantity }}개</p>
          </div>
        </div>
...
const uploadImagesToFirebase = async () => {
  ...
  for (let i = 0; i < imageFiles.value.length; i++) {
    if (imageFiles.value[i]) {
      ...
      const storageRef = firebaseRef(firebaseStorage, `uploads/reviews/${uniqueName}`);
      try {
  ...
  const submitReview = async () => {
    if (rating.value === 0 || reviewText.value.length < 5) {
      alert('별점과 리뷰 내용을 모두 입력해주세요. 리뷰는 최소 5자 이상 작성해주세요.');
      return;
    }
