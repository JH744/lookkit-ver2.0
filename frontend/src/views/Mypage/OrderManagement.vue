          ...
          <div class="step">
            <div class="circle-container">
              <div
                :class="[
                  'step-circle',
                  counts.countPending > 0 ? 'blue-circle' : 'white-circle',
                ]"
              ></div>
              <p
                :class="[
                  'step-number',
                  counts.countPending > 0 ? 'white-text' : 'gray-text',
                ]"
              >
                {{ counts.countPending }}
              </p>
          ...
          <div class="step">
            <div class="circle-container">
              <div
                :class="[
                  'step-circle',
                  counts.countShipped > 0 ? 'blue-circle' : 'white-circle',
                ]"
              ></div>
              <p
                :class="[
                  'step-number',
                  counts.countShipped > 0 ? 'white-text' : 'gray-text',
                ]"
              >
                {{ counts.countShipped }}
              </p>
          ...
          <div class="step">
            <div class="circle-container">
              <div
                :class="[
                  'step-circle',
                  counts.countDelivered > 0 ? 'blue-circle' : 'white-circle',
                ]"
              ></div>
              <p
                :class="[
                  'step-number',
                  counts.countDelivered > 0 ? 'white-text' : 'gray-text',
                ]"
              >
          ...
          <div class="step">
            <div class="circle-container">
              <div
                :class="[
                  'step-circle',
                  counts.countCompleted > 0 ? 'blue-circle' : 'white-circle',
                ]"
              ></div>
              <p
                :class="[
                  'step-number',
                  counts.countCompleted > 0 ? 'white-text' : 'gray-text',
                ]"
              >
        ...
        >
          ...
          <div class="product-details">
            ...
            <div class="product-price">
              {{ formatPrice(product.productPrice) }}원 / 수량:
              {{ product.quantity }}
            </div>
          </div>
          <div class="status-section-text">
            ...
            <router-link class="inquiry-button" to="/mypage/inquiry/create"
              >1:1 문의하기</router-link
            >
          </div>
          <div class="rental-grid">
            <p v-show="product.rentalStartDate">
              {{ product.rentalStartDate }}
            </p>
            <img
              v-show="product.rentalStartDate && product.rentalEndDate"
              width="18px"
          ...
          <div class="actions">
            ...
            <a
              :href="
                product.codiId
                  ? `/mypage/review/create?codiId=${product.codiId}`
                  : `/mypage/review/create?productId=${product.productId}`
              "
              class="review-button"
              >리뷰 작성</a
            >
...
const fetchImageForProduct = async (product) => {
  const storagePath = product.productId
    ? `lookkit/products/0${product.productId}/${product.productId}_thumbnail.webp`
    : `lookkit/codi/0${product.codiId}/${product.codiId}_thumbnail.webp`;
